...
function getBusinessDayWindow(referenceNow = getNowInIsrael()) {
  const isBefore3am = referenceNow.hour < 3;
  const baseDate = new Date(Date.UTC(referenceNow.year, referenceNow.month - 1, referenceNow.day));
  if (isBefore3am) {
    baseDate.setUTCDate(baseDate.getUTCDate() - 1);
  }
  const startYear = baseDate.getUTCFullYear();
  const startMonth = String(baseDate.getUTCMonth() + 1).padStart(2, '0');
  const startDay = String(baseDate.getUTCDate()).padStart(2, '0');
  ...
  return {
    ...
    startDateTime: `${startDate} 03:00:00`,
    endDateTime: `${endDate} 03:00:00`
  };
}
...
app.get('/api/my-records', authRequired, async (req, res) => {
  try {
    await ensureAutoCloseSpecialRecords();
    const window = getBusinessDayWindow();
    ...
    const result = await query(
      `SELECT *
       ...
       ORDER BY record_time DESC, id DESC`,
      [req.user.id, window.startDateTime, window.endDateTime]
    );
...
app.post('/api/attendance', authRequired, async (req, res) => {
  try {
    ...
    if (
      ...
      lastRecord &&
      lastRecord.record_type === 'in' &&
      String(lastRecord.work_day_type || '') !== String(workDayType || '')
    ) {
      return res.status(400).json({
        error: `לא ניתן לסגור יום מסוג שונה. יש לסגור את היום בתור "${lastRecord.work_day_type}"`
      });
    }
