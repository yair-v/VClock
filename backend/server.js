...
app.get('/api/admin/dashboard-daily-category', authRequired, adminRequired, async (req, res) => {
  try {
    const rawDate = String(req.query.date || '').trim();
    const workGroupIdRaw = String(req.query.workGroupId || '').trim();
    const targetDate = rawDate || new Date().toISOString().slice(0, 10);
    const workGroupId = workGroupIdRaw ? parseInt(workGroupIdRaw, 10) : null;

    if (!/^\d{4}-\d{2}-\d{2}$/.test(targetDate)) {
      return res.status(400).json({ error: 'תאריך לא תקין' });
    }

    if (workGroupIdRaw && Number.isNaN(workGroupId)) {
      return res.status(400).json({ error: 'קבוצת עבודה לא תקינה' });
    }

    let groupName = 'כל הקטגוריות';
    if (workGroupId) {
      const groupResult = await query(
        `SELECT id, name FROM work_groups WHERE id = $1`,
        [workGroupId]
      );

      if (!groupResult.rows[0]) {
        return res.status(404).json({ error: 'קבוצת העבודה לא נמצאה' });
      }
    ...
    const totalUsersResult = await query(
      `SELECT COUNT(*)::int AS count
       ...
       WHERE u.role = 'employee'
         AND u.is_active = 1
         AND ($1::int IS NULL OR u.work_group_id = $1)`,
      [workGroupId]
    );

    const reportedUsersResult = await query(
      `SELECT COUNT(DISTINCT ar.user_id)::int AS count
       FROM attendance_records ar
       JOIN users u ON u.id = ar.user_id
       WHERE ar.record_type = 'in'
         ...
         AND u.role = 'employee'
         AND u.is_active = 1
         AND ($2::int IS NULL OR u.work_group_id = $2)`,
      [targetDate, workGroupId]
    );

    const totalUsers = totalUsersResult.rows[0]?.count || 0;
    const reportedUsers = reportedUsersResult.rows[0]?.count || 0;
    const missingUsers = Math.max(totalUsers - reportedUsers, 0);
    const percent = totalUsers > 0 ? Number(((reportedUsers / totalUsers) * 100).toFixed(1)) : 0;
