...
export default function EmployeePage() {
  ...
  const user = useMemo(() => JSON.parse(localStorage.getItem('vclock_user') || '{}'), []);

  async function loadStatus() {
    try {
      const data = await apiGet('/attendance/status');
      setStatus(data);
    } catch (err) {
  ...
  async function submitRecord(recordType) {
    ...
    setLoading(true);
    try {
      const data = await apiPost('/attendance/record', {
        recordType,
        workDayType,
        note,
      });
    ...
    <div className="employee-page">
      <div className="phone-card">
        ...
        <div className="info-grid">
          ...
          <div className="info-box full"><strong>סטטוס נוכחי</strong><span>{status?.isCheckedIn ? 'נמצא בעבודה' : 'לא נמצא בעבודה'}</span></div>
        </div>
